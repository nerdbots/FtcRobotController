...
public class PurePursuitRobotMovement5 {
...
//    static final double DISTANCE_THRESHOLD = 2;
    ...
    public PurePursuitRobotMovement5(LinearOpMode opmode) {
        this.opmode = opmode;
...
//            double relativeTurnAngle = MathFunctions.AngleWrapDeg(robotAngleToTarget - 180 + preferredAngle);
            double relativeTurnAngle = MathFunctions.AngleWrapDeg(preferredAngle - (getAngle() + 90));
...
//        robotXdisplacementOpt = -frontDispNoRot * ((2.362 * Math.PI) / 1440);
...
//        double wheelSpeedIPS = wheelSpeedRPS * wheelDiameter * Math.PI; //
    ...
    public void followCurve(ArrayList<CurvePoint> allPoints, double followAngle, double distanceToPark, double parkAngleTarget){

        startTime = elapsedTime.seconds();
        oldTime = startTime;
        distanceToEndPoint = 10;

        while (this.opmode.opModeIsActive() && !this.opmode.isStopRequested() && !distanceTargetReached(distanceToEndPoint)) {
            ...
            CurvePoint followMe = getFollowPointPath(allPoints, new Point(robotPositionXYV[4], robotPositionXYV[5]),
                    allPoints.get(0).followDistance);

            CurvePoint endPoint = getEndPoint(allPoints, new Point(robotPositionXYV[4], robotPositionXYV[5]),
                    allPoints.get(0).followDistance);
            ...
            if(distanceToEndPoint < distanceToPark){
                goToPositionEndPP(endPoint.x, endPoint.y, 1.0, parkAngleTarget, 0.2);
            }
            else {
...
//        }
    ...
    private CurvePoint getEndPoint(ArrayList<CurvePoint> pathPoints, Point robotLocation, double followRadius){
...
//        ArrayList<Point> endPathPoint = MathFunctions.lineCircleIntersection(robotLocation, followRadius, startPath.toPoint(), endPath.toPoint());
//        for(Point thisEndPathPoint : endPathPoint){
...
//        robotAngleToTarget = MathFunctions.AngleWrapDeg(robotTargetAngle - getAngle());
        ...
        if (Math.abs(relativeTurnAngle) > 0){
            useZPID = false;
        }
        else if (Math.abs(relativeTurnAngle) < -1){
            useZPID = true;
        }

        if (useZPID){
            zPIDAngle = 90 + getAngle();
            robotTurnSpeed = NerdPID_PurePursuit.zPowerDrive(relativeTurnAngle, zPIDAngle, loopTime);
        }
        else if (!useZPID){
            robotTurnSpeed = turnSpeed;
        }
...
//        rearRightMotor.setPower(0);
    ...
    public void goToPositionEndPP(double x, double y, double movementSpeed, double targetAngleForPark, double turnSpeed){
//        startTime = runtime;
...
//        deltaTime = currentTime - startTime;
            ...
            if (distanceToTarget < 35 && distanceToTarget > 15){
//                robotSpeed = powerToSpeed ((movementSpeed - 0.25));
//                decelRate = Math.pow(robotSpeed, 2) / (2 * 30); //later need to define the transition as variable, currently 30 inches
//                robotTargetSpeedIPS = Math.sqrt(2 * decelRate * (distanceToTarget - 8));
//                robotTargetSpeed = speedToPower(robotTargetSpeedIPS);
                robotTargetSpeed = movementSpeed * (distanceToTarget - 15) / 30;
                prevDistanceToTarget = 20;
...
//                if (robotTargetSpeedPID > 0 && robotTargetSpeedPID < 0.2){
//                    robotTargetSpeed = 0.2;
//                } else if (robotTargetSpeedPID > -0.2 && robotTargetSpeedPID < 0){
//                    robotTargetSpeed = -0.2;
//                } else
...
//            robotTargetSpeed = NerdPID_PurePursuit.movementSpeedPID(distanceToTarget, prevDistanceToTarget, deltaTime);
//            prevDistanceToTarget = distanceToTarget;
...
//            }
            ...
            // channels to record to debug optical encoder field centric driving
            if (debugFlag) {
                RobotLog.d("goToPositionEndPP - deltaTime %f, distanceToTarget %f, robotTargetSpeed %f, robotTargetSpeedIPS %f, robotSpeed %f, decelRate %f, xPositionOptical %f, yPositionOptical %f, frontLeftMotorPower %f, rearRightMotorPower %f, frontRightMotorPower %f, rearLeftMotorPower %f, targetParkAngle %f, zPIDAngle %f, xPower %f, yPower %f, zPower %f, robotTargetSpeedPID %f, prevDistanceToTarget %f",
                        loopTime, distanceToTarget, robotTargetSpeed, robotTargetSpeedIPS, robotSpeed, decelRate, xPositionOpt, yPositionOpt, motorSpeedCommand[0], motorSpeedCommand[3], motorSpeedCommand[1], motorSpeedCommand[2], targetParkAngle, zPIDAngle, xPower, yPower, zPower, robotTargetSpeedPID, prevDistanceToTarget);
